...
def multiple_cutoff_rankings(
    scores, ids, cutoff, invert=True, return_full_rankings=False
):
    ...
    partition = np.argpartition(
        scores, cutoff - 1, axis=1
    )  # efficient sorting: this function first does a pivoting kind of operation in quick sort
    sorted_partition = np.argsort(
        scores[ind[:, None], partition[:, :cutoff]], axis=1
    )  # this line then sorts only the top cutoff elements of each row

    rankings = ids[
        partition[ind[:, None], sorted_partition]
    ]  # shape = (num_samples, cutoff)
...
def data_split_rank_and_invert(scores, data_split, tiebreakers=None):
    ...
    for qid in np.arange(data_split.num_queries()):
        ...
        if tiebreakers is not None:
            ...
            (ranking[s_i:e_i], inverted[s_i:e_i]) = rank_and_invert(
                q_scores, tiebreakers=q_tiebreakers
            )
        else:
            (ranking[s_i:e_i], inverted[s_i:e_i]) = rank_and_invert(q_scores)
